    ...
    def generate_zip():
        try:
            with app.app_context():
                ...
                zip_path = os.path.join(app.config['ZIP_OUTPUT_PATH'], zip_filename)

                with open(zip_path, 'wb') as f:
                    for chunk in stream_zip(member_files(images)):
                        f.write(chunk)

                redis_client.setex(f'zip_task:{task_id}', 3600, f'ready:{zip_filename}')

        except Exception as e:
            app.logger.exception('ZIP generation failed for task %s', task_id)
...
def member_files(images):
    for image in images:
        modified_at = image.uploaded_at
        perms = 0o600

        def file_bytes(path=image.file_path):
